...
export const ANGULAR_GUIDANCE = [
  ...
  'A minimal `ng new` scaffold has almost no application code, so a footprint measured against one is a floor, not a typical expectation: T06 measured a major-version `ng update --allow-dirty` on such a scaffold changing only 2 files (`package.json` and the lockfile) and nothing at all under `src/`, because every migration schematic found nothing to rewrite there. A real application — with route guards, resolvers, or `moduleId` usage — will change many more files under `src/**`. Never treat a small measured count as the ceiling of what a migration may touch.',
