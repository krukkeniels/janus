...
| `start_round` | `go: next_task`, `too_many: blocked` | `too_many` when `round >= allowed` (checked before incrementing, so a `retry` at `blocked` continues with the next round); else `round += 1`, `task_index = 0`, `finished = []` |
...
**Nested loops.** Loops compose by edges: the example has the majors (`major_done -- next --> next_major`), the rounds (six edges carry findings back to `start_round`, and `blocked`'s `retry` is the seventh) and the tasks (`task_done --> next_task`) with the CI loop inside (`fix -- ci --> ci`). Each node's visit count keeps the keys apart at any depth: the second major's plan is `plan#2/plan#1`, the fourth round's implement of the first task is `implement#4/implement#1/1`.
